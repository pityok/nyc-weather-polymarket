...
router.get("/api/sim-bets", async (req, res, next) => {
  try {
    const { date, cityId } = signalsQuerySchema.parse(req.query);
    ...
    const edgeWeighted = items.filter((x) => x.strategy === "edgeWeighted");

    res.json({
      date,
      cityId,
      items,
      totalStakeUsd: items.reduce((a, b) => a + b.stakeUsd, 0),
      strategies: {
        equal33: { items: equal33, totalStakeUsd: equal33.reduce((a, b) => a + b.stakeUsd, 0) },
        edgeWeighted: { items: edgeWeighted, totalStakeUsd: edgeWeighted.reduce((a, b) => a + b.stakeUsd, 0) },
      },
    });
  } catch (error) {
    next(error);
